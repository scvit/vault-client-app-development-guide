...
public class VaultClient {
    ...
    try {
      ...
      try (CloseableHttpResponse response = httpClient.execute(request)) {
        if (response.getCode() == 200) {
          ...
          if (dataNode != null) {
            JsonNode ttlNode = responseNode.get("ttl");
            int ttl = ttlNode != null ? ttlNode.asInt() : 3600; // 기본값 1시간
   ...
   * KV 시크릿 버전 정보 조회
   */
  public int getKvSecretVersion() {
    SecretCache cachedSecret = secretCaches.get("kv:" + config.getKvPath());
    return cachedSecret != null ? cachedSecret.getVersion() : -1;
  }

  /**
   * Database Dynamic 시크릿 TTL 정보 조회 (실시간)
   */
  public int getDatabaseDynamicSecretTtl() {
    SecretCache cachedSecret = secretCaches.get("db_dynamic:" + config.getDatabaseDynamicRoleId());
    if (cachedSecret == null) {
      return -1;
    }

    // 캐시된 TTL에서 경과 시간을 빼서 현재 남은 TTL 계산
    long elapsedSeconds = java.time.Duration.between(cachedSecret.getTimestamp(), LocalDateTime.now()).getSeconds();
    int remainingTtl = (int) (cachedSecret.getTtl() - elapsedSeconds);
    return Math.max(0, remainingTtl);
  }

  /**
   * Database Static 시크릿 TTL 정보 조회 (실시간)
   */
  public int getDatabaseStaticSecretTtl() {
    SecretCache cachedSecret = secretCaches.get("db_static:" + config.getDatabaseStaticRoleId());
    if (cachedSecret == null) {
      return -1;
    }
